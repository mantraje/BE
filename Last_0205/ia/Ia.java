...
public class Ia extends Robot {
    ...
    public Ia(String monNXT,int posRobotIAPrevious, int init, Graphe g, String last, int capaciteCofrre,String nomC,Bluetooth b) throws InterruptedException, IOException {
        super(monNXT,posRobotIAPrevious, init, g, last,capaciteCofrre,nomC,b);
    ...
    private List<Couple> creerFils(Couple current, int but, List<Couple> vu){
        ...
        for (int curFils : graphe.getVoisin(curSommet)){
            ...
            if ((curFils==(log.dernierPosCoop(nomCoop))&&(!graphe.estHopital(log.dernierPosCoop(nomCoop))))){gfils+=1000;}
