...
public class Ia extends Robot {
    ...
    private void lancementIA(){
        ...
        for (i = 1; i < cheminResult.size() - 1; i++) {
            ...
            while((cheminResult.get(i + 1) == log.dernierPosCoop(nomCoop)) || (cheminResult.get(i + 2) == log.dernierPosCoop(nomCoop))|| (cheminResult.get(i + 3) == log.dernierPosCoop(nomCoop))){
                sem.release();
                try {
                    sem.acquire();
                    sleep(500);
    ...
    private List<Couple> creerFils(Couple current, int but, List<Couple> vu){
        ...
        for (int curFils : graphe.getVoisin(curSommet)){
            ...
            if ((curFils==(log.dernierPosCoop(nomCoop))&&((!graphe.estHopital(log.dernierPosCoop(nomCoop)))||(!graphe.estVictime(log.dernierPosCoop(nomCoop)))))){gfils+=100;}
            sem.release();
    ...
    private void esquivePilote(){
        ...
        while(!fini) {
            ...
            cheminEsquive.addAll(AEtoile(attente, new LinkedList<>(), fristListHopital(graphe.getHopitaux(), currentPosPilote)));
            if (cheminEsquive.contains(currentPos)) {
                for (i = 1; i < cheminEsquive.size() - 1; i++) {
                    envoieMouvement(i, !frist, cheminEsquive);
                    frist = false;
                }
                currentPos = graphe.next(previousPos, previousPos = currentPos, lastDeplacement);
                if (graphe.estIntersec(currentPos)) {
                    sent("20l");
                } else {
                    sent("20s");
